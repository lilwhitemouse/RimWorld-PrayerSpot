...
namespace LWM.PrayerSpot {
    public class JoyGiver_Spiritual_Simple : JoyGiver_InPrivateRoom {
        ...
        public override Job TryGiveJob(Pawn pawn) {
            ...
            mapPrayerSpots.AddRange(pawn.Map.listerBuildings.AllBuildingsColonistOfDef(PrayerSpotDirectionalDef));
            Building spot = null;
            ...
            if ((r1 = pawn?.ownership?.OwnedRoom)!=null) {
                ...
                for (int i=0; i<l.Count; i++) {
                    if (l[i].def == PrayerSpotDef ||
                        l[i].def == PrayerSpotDirectionalDef) {
                        hasSpotInRoom=true;
                        break;
                    }
                }
